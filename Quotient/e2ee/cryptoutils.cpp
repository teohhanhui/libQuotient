...
{
    auto edKeyResult = types::ed25519_secret_key_from_base64(rust::String(key.data(), key.size()));
    //TODO: edKeyResult error handling
    auto edKey = types::ed25519_secret_key_from_base64_result_value(std::move(edKeyResult));
    auto signature = edKey->sign(rust::Slice<const std::uint8_t>((const unsigned char *) data.data(), data.size()))->to_base64();
